def fetch_model_logs():
    try:
        r = requests.get("http://localhost:8001/model/logs")
        r.raise_for_status()
        return r.json()
...
def fetch_model_errors():
    try:
        r = requests.get("http://localhost:8001/model/errors")
        r.raise_for_status()
        return r.json()
    except Exception as e:
        return []
...
def batch_predict_from_csv(contents):
    ...
    try:
        ...
        with open(tmp_path, 'rb') as f:
            ...
            retrain_resp = requests.post("http://localhost:8001/model/upload_and_retrain", files=files)
        ...
        r = requests.post("http://localhost:8001/model/predict_batch", json=data)
...
def open_trade(symbol, direction, amount, entry_price, tp_pct, sl_pct):
    try:
        r = requests.post("http://localhost:8001/trade", json={
            "symbol": symbol,
            "direction": direction,
            "amount": amount,
...
def fetch_notifications():
    try:
        r = requests.get("http://localhost:8001/notifications")
        r.raise_for_status()
        return r.json()
    except Exception:
        return []
...
def run_backtest(symbol, initial_capital):
    try:
        ...
        r = requests.post("http://localhost:8001/backtest", json={
            "symbol": symbol,
            "data": [],
...
def fetch_backtests():
    try:
        r = requests.get("http://localhost:8001/backtest/results")
        r.raise_for_status()
        return r.json()
    except Exception:
        return []
...
def fetch_ml_prediction(features):
    try:
        r = requests.post("http://localhost:8001/model/predict_batch", json=[features])
        r.raise_for_status()
...
def fetch_analytics():
    try:
        r = requests.get("http://localhost:8001/trades/analytics")
        r.raise_for_status()
        return r.json()
    except Exception:
...
def fetch_trades():
    try:
        r = requests.get("http://localhost:8001/trades")
        r.raise_for_status()
        return r.json()
    except Exception:
        return []
...
def close_trade(trade_id, close_price):
    try:
        r = requests.post(f"http://localhost:8001/trades/{trade_id}/close", params={"close_price": close_price})
        r.raise_for_status()
        return r.json()
    except Exception:
...
def cancel_trade(trade_id):
    try:
        r = requests.post(f"http://localhost:8001/trades/{trade_id}/cancel")
        r.raise_for_status()
        return r.json()
    except Exception:
...
def activate_trade(trade_id):
    try:
        r = requests.post(f"http://localhost:8001/trades/{trade_id}/activate")
        r.raise_for_status()
        return r.json()
    except Exception:
...
def safety_check(symbol, direction, amount, entry_price, tp_pct, sl_pct):
    try:
        ...
        r = requests.post("http://localhost:8001/safety/check", json=payload)
        r.raise_for_status()
        return r.json()
    except Exception as e:
...
def fetch_feature_importance():
    try:
        r = requests.get("http://localhost:8001/model/feature_importance")
        r.raise_for_status()
        return r.json()
    except Exception as e:
        return {"error": str(e)}
...
def fetch_model_metrics():
    try:
        r = requests.get("http://localhost:8001/model/metrics")
        r.raise_for_status()
        return r.json()
    except Exception as e:
        return {"error": str(e)}
...
def mark_notification_read(notification_id):
    try:
        r = requests.post("http://localhost:8001/notifications/mark_read", json=notification_id)
        r.raise_for_status()
...
def delete_notification(notification_id):
    try:
        r = requests.delete(f"http://localhost:8001/notifications/{notification_id}")
        r.raise_for_status()
        return r.json() if r.content else {"ok": True}
    except Exception as e:
        return {"error": str(e)}
...
def fetch_system_status():
    try:
        r = requests.get("http://localhost:8001/system/status")
        r.raise_for_status()
        return r.json()
    except Exception as e:
        return {"error": str(e)}
