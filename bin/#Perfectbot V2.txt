...
with st.sidebar:
    ...
    if st.button("🔄 Reset Virtual Balance", key="sidebar_reset_virtual_balance"):
        st.session_state.virtual_balance = 10000.0
        st.session_state.active_trades = {}
        st.session_state.trade_history = []
        st.session_state.notifications = []
...
with auto_tab:
    ...
    if log:
        ...
        st.info("No auto trades yet.")

    # --- Auto Trading Core Logic ---
    import time
    import pandas as pd
    if auto_enabled:
        # Try to get latest features for the selected symbol from the most recent trade
        features = None
        # Try to get features from most recent trade
        for trade in reversed(st.session_state.trade_history):
            if trade.get('symbol', '').lower() == auto_symbol.lower() and 'features' in trade and trade['features']:
                ...
                break
        # If not found, try to build features from latest available DataFrame (e.g., dashboard, scanner, or backtest)
        if features is None:
            df_source = None
            # Try to use backtest_data if available
            if 'backtest_data' in st.session_state:
                df_source = st.session_state['backtest_data']
            # You can add more sources here (dashboard_df, scanner_results, etc.)
            if df_source is not None:
                # Filter for the selected symbol if possible
                if 'symbol' in df_source.columns:
                    df_symbol = df_source[df_source['symbol'].str.lower() == auto_symbol.lower()]
                else:
                    df_symbol = df_source
                if not df_symbol.empty:
                    # Use the last row for features
                    from collections import OrderedDict
                    # Use feature engineering pipeline if available
                    try:
                        # Try to use get_consistent_features if defined
                        if 'get_consistent_features' in globals():
                            X_scaled, scaler, feature_cols = get_consistent_features(df_symbol, fit_scaler=False)
                            features = OrderedDict(zip(feature_cols, X_scaled[-1]))
                        else:
                            # Fallback: use all numeric columns
                            last_row = df_symbol.iloc[-1]
                            features = OrderedDict((col, last_row[col]) for col in df_symbol.columns if isinstance(last_row[col], (int, float)))
                    except Exception as e:
                        features = None
        if features is None:
            st.warning("Auto Trading requires recent features for the selected symbol. Please execute at least one trade or analysis for this symbol, or ensure historical data is available.")
        else:
            ...
            now = time.time()
            # Run at most once every 10 seconds
            if now - st.session_state['last_auto_trade_time'] > 10:
                st.session_state['last_auto_trade_time'] = now
                try:
                    price = fetch_binance_price(auto_symbol)
                    if price:
                        # Use AdaptiveEnsemble for prediction if available
                        prediction = None
                        if hasattr(st.session_state, 'adaptive_ensemble') and hasattr(st.session_state.adaptive_ensemble, 'predict'):
                            ...
                            # Assume prediction_val > 0 is LONG, < 0 is SHORT, abs(prediction_val) is confidence
                            if prediction_val is not None:
                                direction = 'LONG' if prediction_val > 0 else 'SHORT'
                                confidence = min(100, max(0, abs(prediction_val) * 100))
                                ...
                                if confidence >= auto_min_conf:
                                    ...
                                    if not active:
                                        ...
                                        msg = f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] Opened {direction} {auto_symbol.upper()} @ ${price:.4f} | Amount: ${amount:.2f} | TP: {auto_tp:.2f}% | SL: {auto_sl:.2f}% | Conf: {confidence:.1f}% (Ensemble)"
                                        st.session_state['auto_trading_log'].append(msg)
                                        st.rerun()
                                    else:
                                        # Optionally, close trade if signal reverses
                                        for tid, t in list(st.session_state.active_trades.items()):
                                            if t['symbol'].lower() == auto_symbol.lower() and t['status'] == 'OPEN':
                                                ...
                                                if (prev_dir == 'LONG' and direction == 'SHORT') or (prev_dir == 'SHORT' and direction == 'LONG'):
                                                    ...
                                                    del st.session_state.active_trades[tid]
                                                    msg = f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] Closed {prev_dir} {auto_symbol.upper()} @ ${price:.4f} | P&L: ${t['pnl']:.2f} (Auto)"
                                                    st.session_state['auto_trading_log'].append(msg)
                                                    st.rerun()
                                else:
                                    st.session_state['auto_trading_log'].append(f"[INFO] Confidence {confidence:.1f}% below threshold for {auto_symbol.upper()}.")
                            else:
                                st.session_state['auto_trading_log'].append(f"[INFO] No valid ensemble prediction for {auto_symbol.upper()}.")
                        else:
                            st.session_state['auto_trading_log'].append(f"[ERROR] AdaptiveEnsemble not available for prediction.")
                except Exception as e:
                    st.session_state['auto_trading_log'].append(f"[ERROR] {e}")
