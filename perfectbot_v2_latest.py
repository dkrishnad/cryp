...
from perfectbot_v2_latest import AdvancedBacktester  # Use direct import at top

# --- Caching for heavy data processing and backtesting ---
@st.cache_data(show_spinner=False)
def cached_generate_synthetic_data(symbol, timeframe, seed=42):
    import numpy as np
    import pandas as pd
...
@st.cache_data(show_spinner=False)
def cached_run_backtest(backtest_type, data, initial_capital, trading_fee, slippage, position_size, tp_ratio, sl_ratio, train_window, test_window, backtest_symbol):
    import numpy as np
    import pandas as pd
...
with st.sidebar:
    ...
    reset_balance_triggered = st.button("🔄 Reset Virtual Balance", key="sidebar_reset_virtual_balance")

    def reset_virtual_balance_bg():
        st.session_state.virtual_balance = 10000.0
        st.session_state.active_trades = {}
        st.session_state.trade_history = []
        st.session_state.notifications = []
        st.session_state['sidebar_reset_virtual_balance_result'] = 'success'
        st.session_state['sidebar_reset_virtual_balance_running'] = False

    if reset_balance_triggered and not st.session_state.get('sidebar_reset_virtual_balance_running', False):
        st.session_state['sidebar_reset_virtual_balance_running'] = True
        thread = threading.Thread(target=reset_virtual_balance_bg)
        thread.start()

    if st.session_state.get('sidebar_reset_virtual_balance_running', False):
        st.info("Resetting virtual balance...")
    if st.session_state.get('sidebar_reset_virtual_balance_result') == 'success':
        st.success("Virtual balance reset to $10,000!")
        st.session_state['sidebar_reset_virtual_balance_result'] = None
    ...
    test_db_triggered = st.button("Test DB Write", key="sidebar_test_db_write")
    def test_db_write_bg():
        dummy_trade = {
            'id': 'test1234',
            'symbol': 'btcusdt',
            'direction': 'LONG',
        ...
        st.session_state['sidebar_test_db_write_result'] = 'success'
        st.session_state['sidebar_test_db_write_running'] = False

    if test_db_triggered and not st.session_state.get('sidebar_test_db_write_running', False):
        st.session_state['sidebar_test_db_write_running'] = True
        thread = threading.Thread(target=test_db_write_bg)
        thread.start()

    if st.session_state.get('sidebar_test_db_write_running', False):
        st.info("Writing dummy trade to DB...")
    if st.session_state.get('sidebar_test_db_write_result') == 'success':
        st.success("Dummy trade written to DB!")
        st.session_state['sidebar_test_db_write_result'] = None
    test_ml_triggered = st.button("Test ML", key="sidebar_test_ml")
    def test_ml_bg():
        import pandas as pd
        df = pd.DataFrame({
        ...
        st.session_state['sidebar_test_ml_result'] = 'success'
        st.session_state['sidebar_test_ml_running'] = False

    if test_ml_triggered and not st.session_state.get('sidebar_test_ml_running', False):
        st.session_state['sidebar_test_ml_running'] = True
        thread = threading.Thread(target=test_ml_bg)
        thread.start()

    if st.session_state.get('sidebar_test_ml_running', False):
        st.info("ML retraining in progress...")
    if st.session_state.get('sidebar_test_ml_result') == 'success':
        st.success("ML retraining triggered with dummy data!")
        st.session_state['sidebar_test_ml_result'] = None

    st.markdown("<hr style='border:1px solid #222;'>", unsafe_allow_html=True)
    st.markdown("<h4 style='color:#00aaff;'>🔍 Feature Importance</h4>", unsafe_allow_html=True)
    show_fi_triggered = st.button("Show Feature Importance", key="sidebar_show_feature_importance")
    def show_fi_bg():
        ...
        if models:
            ...
            if importances is not None and feature_cols:
                ...
                st.session_state['sidebar_show_fi_result'] = fi_df
            else:
                st.session_state['sidebar_show_fi_result'] = 'info: Feature importances not available. Train a model first.'
        else:
            st.session_state['sidebar_show_fi_result'] = 'info: No trained model found.'
        st.session_state['sidebar_show_fi_running'] = False

    if show_fi_triggered and not st.session_state.get('sidebar_show_fi_running', False):
        st.session_state['sidebar_show_fi_running'] = True
        thread = threading.Thread(target=show_fi_bg)
        thread.start()

    if st.session_state.get('sidebar_show_fi_running', False):
        st.info("Calculating feature importance...")
    if isinstance(st.session_state.get('sidebar_show_fi_result'), str):
        if st.session_state['sidebar_show_fi_result'].startswith('info:'):
            st.info(st.session_state['sidebar_show_fi_result'].replace('info: ',''))
        st.session_state['sidebar_show_fi_result'] = None
    elif st.session_state.get('sidebar_show_fi_result') is not None:
        st.dataframe(st.session_state['sidebar_show_fi_result'], use_container_width=True)
        st.session_state['sidebar_show_fi_result'] = None
    if isinstance(st.session_state.get('sidebar_show_fi_result'), str):
        if st.session_state['sidebar_show_fi_result'].startswith('info:'):
            st.info(st.session_state['sidebar_show_fi_result'].replace('info: ',''))
    ...
    run_sample_backtest_triggered = st.button("Run Backtest on Sample Data", key="sidebar_run_backtest_sample")
    def run_sample_backtest_bg():
        df = None
        try:
        ...
        if df is not None and len(df) > 30:
            ...
            st.session_state['sidebar_run_backtest_sample_result'] = {'win_rate': win_rate, 'avg_pnl': avg_pnl}
        else:
            st.session_state['sidebar_run_backtest_sample_result'] = 'info: Not enough historical data for backtest.'
        st.session_state['sidebar_run_backtest_sample_running'] = False

    if run_sample_backtest_triggered and not st.session_state.get('sidebar_run_backtest_sample_running', False):
        st.session_state['sidebar_run_backtest_sample_running'] = True
        thread = threading.Thread(target=run_sample_backtest_bg)
        thread.start()

    if st.session_state.get('sidebar_run_backtest_sample_running', False):
        st.info("Running sample backtest...")
    if isinstance(st.session_state.get('sidebar_run_backtest_sample_result'), dict):
        win_rate = st.session_state['sidebar_run_backtest_sample_result']['win_rate']
        avg_pnl = st.session_state['sidebar_run_backtest_sample_result']['avg_pnl']
        st.write(f"Backtest Win Rate: {win_rate:.2f}%")
        st.write(f"Average P&L per Trade: {avg_pnl:.2f}")
        st.session_state['sidebar_run_backtest_sample_result'] = None
    elif isinstance(st.session_state.get('sidebar_run_backtest_sample_result'), str):
        if st.session_state['sidebar_run_backtest_sample_result'].startswith('info:'):
            st.info(st.session_state['sidebar_run_backtest_sample_result'].replace('info: ',''))
        st.session_state['sidebar_run_backtest_sample_result'] = None

    st.markdown("<hr style='border:1px solid #222;'>", unsafe_allow_html=True)
    st.markdown("<h4 style='color:#00aaff;'>🧹 Prune Old Trades</h4>", unsafe_allow_html=True)
    prune_trades_triggered = st.button("Prune Trades (Keep Last 1000)", key="sidebar_prune_trades")
    def prune_trades_bg():
        import sqlite3
        try:
            conn = sqlite3.connect(DB_PATH)
            ...
            conn.commit()
            conn.close()
            st.session_state['sidebar_prune_trades_result'] = True
        except Exception as e:
            st.session_state['sidebar_prune_trades_result'] = False
            st.error(f"Prune failed: {e}")
        st.session_state['sidebar_prune_trades_running'] = False

    if prune_trades_triggered and not st.session_state.get('sidebar_prune_trades_running', False):
        st.session_state['sidebar_prune_trades_running'] = True
        thread = threading.Thread(target=prune_trades_bg)
        thread.start()

    if st.session_state.get('sidebar_prune_trades_running', False):
        st.info("Pruning old trades...")
    if st.session_state.get('sidebar_prune_trades_result'):
        st.success("Old trades pruned. Only the last 1000 remain.")
        st.session_state['sidebar_prune_trades_result'] = None
...
with auto_tab:
    st.header("🤖 Auto Trading (Virtual)")
    ...
    st.markdown("<div class='section-title'>⚙️ Auto Trading Settings</div>", unsafe_allow_html=True)
    ...
    # --- Live Log ---
    st.markdown("<div class='section-title'>📜 Auto Trading Log</div>", unsafe_allow_html=True)
    ...
    if auto_enabled:
        ...
        try:
            ...
            if current_price:
                ...
                st.success(f"✅ Generated real-time features for {auto_symbol} at ${current_price:.4f}")
            ...
            try:
                ...
                for trade in reversed(st.session_state.trade_history):
                    if trade.get('symbol', '').lower() == auto_symbol.lower() and 'features' in trade and trade['features']:
                        ...
                        st.info(f"✅ Using features from recent trade for {auto_symbol}")
                        break
                ...
                if features is None and 'backtest_data' in st.session_state:
                    ...
                    if not df_source.empty:
                        ...
                        st.info(f"✅ Using backtest data features for {auto_symbol}")
        ...
        if features is None:
            ...
            st.info("✅ Using synthetic features for auto trading demo")
        
        if features is None:
            st.error("❌ Could not generate features for auto trading. Please try refreshing or selecting a different symbol.")
        else:
            ...
            if not hasattr(st.session_state, 'adaptive_ensemble') or st.session_state.adaptive_ensemble is None:
                try:
                    ...
                    st.success("✅ Auto trading ensemble initialized")
                                ...
                                if confidence >= auto_min_conf:
                                    ...
                                    if not active:
                                        ...
                                        if amount > 0:  # Ensure positive amount
                                            ...
                                            msg = f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] 🚀 Opened {direction} {auto_symbol.upper()} @ ${price:.4f} | Amount: ${amount:.2f} | TP: {auto_tp:.1f}% | SL: {auto_sl:.1f}% | Confidence: {confidence:.1f}%"
                                            st.session_state['auto_trading_log'].append(msg)
                                            st.success(f"✅ Auto trade opened: {direction} {auto_symbol}")
                                    else:
                                        ...
                                        for tid, t in list(st.session_state.active_trades.items()):
                                            if t['symbol'].lower() == auto_symbol.lower() and t['status'] == 'OPEN':
                                                ...
                                                if (prev_dir == 'LONG' and direction == 'SHORT') or (prev_dir == 'SHORT' and direction == 'LONG'):
                                                    ...
                                                    if t['pnl'] > 0:
                                                        st.success(f"✅ Profitable auto trade closed: +${t['pnl']:.2f}")
                                                    else:
                                                        st.warning(f"⚠️ Auto trade closed at loss: ${t['pnl']:.2f}")
                                else:
                                    # Log low confidence
                                    if len(st.session_state['auto_trading_log']) == 0 or "below threshold" not in st.session_state['auto_trading_log'][-1]:
                                        st.session_state['auto_trading_log'].append(f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] ⏳ Confidence {confidence:.1f}% below {auto_min_conf}% threshold for {auto_symbol.upper()}")
                            else:
                                st.session_state['auto_trading_log'].append(f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] ❌ No valid prediction generated for {auto_symbol.upper()}")
                                
                        except Exception as pred_error:
                            st.session_state['auto_trading_log'].append(f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] ❌ Prediction error: {pred_error}")
                    else:
                        st.session_state['auto_trading_log'].append(f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] ❌ Trading ensemble not available")
                        
                except Exception as e:
                    st.session_state['auto_trading_log'].append(f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] ❌ Auto trading error: {e}")
            else:
                # Show time until next analysis
                time_left = 5 - (now - st.session_state['last_auto_trade_time'])
                st.info(f"⏱️ Next analysis in {time_left:.1f} seconds...")

    # --- Auto Trading Summary ---
    st.markdown("<div class='section-title'>📈 Auto Trading Summary</div>", unsafe_allow_html=True)
...
with decimal_tab:
    ...
    long_triggered = col1.button(f"🟢 LONG {symbol.upper()}", key=f"long_{symbol}")
    short_triggered = col2.button(f"🔴 SHORT {symbol.upper()}", key=f"short_{symbol}")
    trade_result = None
    if long_triggered:
        trade_id = open_virtual_trade(symbol, "LONG", st.session_state['capital'], price, target_pct, quick_sl_percentage)
        trade_result = f"LONG trade placed for {symbol.upper()} at ${price:.4f} (ID: {trade_id})"
        st.session_state['decimal_scalping_last_trade'] = trade_result
    elif short_triggered:
        trade_id = open_virtual_trade(symbol, "SHORT", st.session_state['capital'], price, target_pct, quick_sl_percentage)
        trade_result = f"SHORT trade placed for {symbol.upper()} at ${price:.4f} (ID: {trade_id})"
        st.session_state['decimal_scalping_last_trade'] = trade_result
    if 'decimal_scalping_last_trade' in st.session_state:
        st.info(st.session_state['decimal_scalping_last_trade'])
        st.session_state['decimal_scalping_last_trade'] = None
    ...
    if not symbol or not symbol.lower().endswith('usdt'):
        ...
        with st.spinner(f"Fetching scanner prices and predictions for {symbol.upper()}..."):
            ...
            if price:
                ...
                trade_futures_triggered = st.button(f"Trade {row['symbol']} {row['entry_price']:.10f}", key=f"trade_{row['symbol']}_futures")
                trade_result = None
                if trade_futures_triggered:
                    trade_id = open_virtual_trade(row['symbol'], row['direction'], st.session_state['capital'], row['entry_price'], tp_pct, sl_pct)
                    trade_result = f"{row['direction']} trade placed for {row['symbol']} at ${row['entry_price']:.4f} (ID: {trade_id})"
                    st.session_state['futures_scanner_last_trade'] = trade_result
                if 'futures_scanner_last_trade' in st.session_state:
                    st.info(st.session_state['futures_scanner_last_trade'])
                    st.session_state['futures_scanner_last_trade'] = None
            else:
...
with dashboard_tab:
    ...
    if st.session_state.active_trades:
        ...
        if trades_data:
            ...
            # Emergency close all button (stateless)
            close_all_triggered = st.button("❗ Close All Trades", key="close_all_trades", help="Close all open trades immediately. Use for emergency exit.")
            if close_all_triggered:
                for trade_id in list(st.session_state.active_trades.keys()):
                    st.session_state.active_trades[trade_id]['status'] = 'CLOSED_MANUAL'
                st.session_state['dashboard_last_action'] = 'close_all_trades'
            if st.session_state.get('dashboard_last_action') == 'close_all_trades':
                st.success("All trades closed.")
                st.session_state['dashboard_last_action'] = None
            # Manual close buttons (stateless)
            st.subheader("Manual Close")
            cols = st.columns(min(4, len(trades_data)))
            for i, trade in enumerate(trades_data):
                with cols[i % 4]:
                    close_trade_triggered = st.button(f"Close {trade['Status']}", key=f"close_{i}")
                    if close_trade_triggered:
                        trade_id = list(st.session_state.active_trades.keys())[i]
                        st.session_state.active_trades[trade_id]['status'] = 'CLOSED_MANUAL'
                        st.session_state['dashboard_last_action'] = f'close_{trade_id}'
                    if st.session_state.get('dashboard_last_action') == f'close_{trade_id}':
    ...
    if st.session_state.pending_trades:
        ...
        if pending_data:
            ...
            # Cancel pending orders section (stateless)
            st.subheader("Cancel Pending Orders")
            cols = st.columns(min(4, len(pending_data)))
            for i, trade in enumerate(pending_data):
                col = cols[i % len(cols)]
                with col:
                    cancel_pending_triggered = st.button(f"Cancel {trade['ID']}", key=f"cancel_{trade['ID']}")
                    if cancel_pending_triggered:
                        trade_id = trade['ID']
                        if trade_id in st.session_state.pending_trades:
                            st.session_state.pending_trades[trade_id]['status'] = 'CANCELLED'
                        st.session_state['dashboard_last_action'] = f'cancel_{trade_id}'
                    if st.session_state.get('dashboard_last_action') == f'cancel_{trade_id}':
                        st.success(f"Pending order {trade_id} cancelled.")
                        st.session_state['dashboard_last_action'] = None
    else:
...
with scanner_tab:
    ...
    scan_triggered = st.button("🔍 Scan Selected Pair", type="primary")
    if scan_triggered:
        st.session_state['pending_scanner_scan'] = True
    if st.session_state.get('pending_scanner_scan', False) and not st.session_state.get('scanner_last_scan'):
        symbol = st.session_state.get('selected_symbol', None)
        if not symbol or not symbol.lower().endswith('usdt'):
            scan_result = "No USDT pair selected for scanning."
            st.session_state['scanner_last_scan'] = scan_result
        else:
            with st.spinner(f"Scanning {symbol.upper()}..."):
                ...
                for idx, row in top_signals.iterrows():
                    ...
                    with col2:
                        trade_scanner_triggered = st.button(f"Trade {row['symbol']} {row['price']:.4f}", key=f"scanner_trade_{row['symbol']}_{idx}")
                        if trade_scanner_triggered:
                            trade_id = execute_trade_from_prediction(row['symbol'], row['direction'], st.session_state['capital'], row['price'], 2.0, 1.0)
                            trade_result = f"{row['direction']} trade placed for {row['symbol']} at ${row['price']:.4f} (ID: {trade_id})"
                            st.session_state['scanner_last_trade'] = trade_result
    if 'scanner_last_scan' in st.session_state:
        st.warning(st.session_state['scanner_last_scan'])
        st.session_state['scanner_last_scan'] = None
        st.session_state['pending_scanner_scan'] = False
    if 'scanner_last_scan' in st.session_state:
        st.warning(st.session_state['scanner_last_scan'])
        st.session_state['scanner_last_scan'] = None
    if 'scanner_last_trade' in st.session_state:
        st.info(st.session_state['scanner_last_trade'])
        st.session_state['scanner_last_trade'] = None
...
with backtest_tab:
    ...
    data_source = st.radio("Select Data Source", 
                          ["📁 Upload High-Quality CSV File", "🎲 Generate Synthetic Data"], 
                          horizontal=True)
    
    use_uploaded_data = False
    
    if data_source == "📁 Upload High-Quality CSV File":
        st.markdown("### 📁 **Advanced CSV Data Upload**")
        st.info("🎯 Upload high-quality trading data with advanced features and detailed timestamps (year/month/day/hour/minute/second)")
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("🔍 Process & Validate High-Quality Data", type="primary"):
                        ...
                        try:
                            ...
                            if 'data_quality_manager' in st.session_state:
                                ...
                                if issues:
                                    ...
                                    st.success("✅ Premium data quality - validation passed perfectly!")
                            ...
                            st.success("🎉 High-quality dataset processed and ready for advanced ML backtesting!")
    ...
    # --- Always show symbol selection at the top of advanced backtesting ---
    st.markdown("### ⚙️ Advanced Backtesting Setup")
    col_symbol, col_timeframe, col_train, col_test = st.columns([2,2,2,2])
    # Use the same symbol list as the sidebar for consistency
    sidebar_symbol_options = [
        'BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'AVAXUSDT', 'DOGEUSDT', 'BNBUSDT', 'MATICUSDT', 'PEPEUSDT', '1000FLOKUSDT',
        '1000SHIBUSDT', '1000XEMUSDT', '1000LUNCUSDT', '1000BONKUSDT', '1000SATSUSDT', '1000RPLUSDT', '1000BABYDOGEUSDT',
        'ORDIUSDT', 'WIFUSDT', 'TUSDT', 'OPLUSDT', 'SUIUSDT', 'ENAUSDT', 'NOTUSDT', 'JUPUSDT', 'KASUSDT', 'TIAUSDT',
        'STXUSDT', 'BLURUSDT', 'GMXUSDT', 'RDNTUSDT', 'HOOKUSDT', 'CYBERUSDT', 'ARKMUSDT', 'SNTUSDT', 'WAVESUSDT',
        'KAIAUSDT', 'ADAUSDT', 'XRPUSDT', 'LTCUSDT', 'LINKUSDT', 'DOTUSDT', 'UNIUSDT', 'BCHUSDT', 'FILUSDT', 'TRXUSDT',
        'ETCUSDT', 'APTUSDT', 'OPUSDT', 'ARBUSDT', 'NEARUSDT', 'ATOMUSDT', 'SANDUSDT', 'MANAUSDT', 'CHZUSDT', 'EGLDUSDT',
        'FTMUSDT', 'ICPUSDT', 'RUNEUSDT', 'SUSHIUSDT', 'AAVEUSDT', 'SNXUSDT', 'CRVUSDT', 'COMPUSDT', 'ENJUSDT', '1INCHUSDT',
        'XMRUSDT', 'ZECUSDT', 'DASHUSDT', 'OMGUSDT', 'YFIUSDT', 'BALUSDT', 'CTKUSDT', 'ANKRUSDT', 'BATUSDT', 'CVCUSDT', 'DGBUSDT'
    ]
    with col_symbol:
    ...
    with col_timeframe:
        timeframe = st.selectbox("Timeframe", ['1m', '5m', '15m', '1h', '4h', '1d'], key='backtest_timeframe')
    with col_train:
        train_window = st.number_input("Training Window (periods)", value=100, min_value=50, max_value=500, key='train_window')
    with col_test:
        test_window = st.number_input("Test Window (periods)", value=20, min_value=10, max_value=100, key='test_window')

    # Add backtest type selection (fixes undefined variable)
    backtest_type = st.selectbox(
        "Backtest Type",
        ["Simple Backtest", "Walk-Forward Validation", "Monte Carlo"],
        key="backtest_type_select"
    )

    # --- Validate Data Quality Button Logic (stateless, robust) ---
    validate_triggered = st.button("🔍 Validate Data Quality", type="secondary")
    if validate_triggered:
        st.session_state['pending_backtest_validate'] = True
    if st.session_state.get('pending_backtest_validate', False):
        with st.spinner("Generating and validating synthetic data..."):
            backtest_symbol = st.session_state.get('backtest_symbol', None)
            if backtest_symbol is None:
                result = "Please select a symbol before generating synthetic data."
                st.session_state['data_quality_result'] = result
            else:
                synthetic_data = cached_generate_synthetic_data(backtest_symbol, timeframe)
                try:
                    issues = st.session_state.data_quality_manager.validate_data(synthetic_data)
                except Exception as e:
                    result = f"Data quality validation failed: {e}"
                    import traceback
                    st.text(traceback.format_exc())
                    issues = [str(e)]
                if issues:
                    result = f"Data quality issues found: {', '.join(issues)}"
                    try:
                        synthetic_data = st.session_state.data_quality_manager.clean_data(synthetic_data)
                    except Exception as e:
                        result = f"Data cleaning failed: {e}"
                        import traceback
                        st.text(traceback.format_exc())
                else:
                    result = "Data quality validation passed!"
                st.session_state['backtest_data'] = synthetic_data
                st.session_state['data_quality_result'] = result
        st.session_state['pending_backtest_validate'] = False
    if 'data_quality_result' in st.session_state:
        st.info(st.session_state['data_quality_result'])
        st.session_state['data_quality_result'] = None

    # --- Run Advanced Backtest Button Logic (stateless, robust) ---
    run_backtest_triggered = st.button("🎯 Run Advanced Backtest", type="primary")
    if run_backtest_triggered:
        st.session_state['pending_backtest_run'] = True
    if st.session_state.get('pending_backtest_run', False):
        try:
            if 'backtest_data' not in st.session_state:
                result = "Please validate data first!"
                st.session_state['backtest_run_result'] = result
            else:
                with st.spinner(f"Running {backtest_type.lower()}..."):
                    backtest_symbol = st.session_state.get('backtest_symbol', None)
                    if backtest_symbol is None:
                        result = "No symbol selected for backtesting. Please select a symbol above."
                        st.session_state['backtest_run_result'] = result
                    else:
                        data = st.session_state.backtest_data.copy()
                        results = cached_run_backtest(
                            backtest_type, data, initial_capital, trading_fee, slippage, position_size, tp_ratio, sl_ratio, train_window, test_window, backtest_symbol
                        )
                        st.session_state['backtest_results'] = results
                        result = f"{backtest_type} completed! Check results below."
                        st.session_state['backtest_run_result'] = result
        except Exception as e:
            result = f"Advanced backtest failed: {e}"
            import traceback
            st.text(traceback.format_exc())
            st.session_state['backtest_run_result'] = result
        st.session_state['pending_backtest_run'] = False
    if 'backtest_run_result' in st.session_state:
        st.info(st.session_state['backtest_run_result'])
        st.session_state['backtest_run_result'] = None
    ...
    analyze_perf_triggered = st.button("📊 Analyze Model Performance")
    if analyze_perf_triggered:
        st.session_state['pending_backtest_analyze_perf'] = True
    if st.session_state.get('pending_backtest_analyze_perf', False):
        try:
            with st.spinner("Analyzing model performance..."):
                ...
                st.session_state['model_performance_df'] = df_performance
                st.session_state['analyze_perf_result'] = 'success'
        except Exception as e:
            st.session_state['analyze_perf_result'] = f'error: {e}'
        st.session_state['pending_backtest_analyze_perf'] = False
    if st.session_state.get('analyze_perf_result') == 'success' and 'model_performance_df' in st.session_state:
        regimes = ['trending', 'ranging', 'volatile']
        col1, col2, col3 = st.columns(3)
        for i, regime in enumerate(regimes):
            with [col1, col2, col3][i]:
                st.write(f"**{regime.title()} Market:**")
                regime_data = st.session_state['model_performance_df'][st.session_state['model_performance_df']['regime'] == regime]
        ...
        st.session_state['analyze_perf_result'] = None
    elif st.session_state.get('analyze_perf_result', '').startswith('error'):
        st.error(st.session_state['analyze_perf_result'])
        st.session_state['analyze_perf_result'] = None
    ...
    col1, col2 = st.columns(2)
    with col1:
        save_results_triggered = st.button("💾 Save Results to Database")
        if save_results_triggered:
            st.session_state['pending_backtest_save_results'] = True
        if st.session_state.get('pending_backtest_save_results', False):
            if st.session_state.backtest_results:
                try:
                    conn = sqlite3.connect(DB_PATH)
                    ...
                    conn.close()
                    st.session_state['backtest_save_result'] = 'success'
                except Exception as e:
                    st.session_state['backtest_save_result'] = f'error: {e}'
            else:
                st.session_state['backtest_save_result'] = 'warning: No results to save!'
            st.session_state['pending_backtest_save_results'] = False
        # Show last result
        if st.session_state.get('backtest_save_result') == 'success':
            st.success("Results saved to database (simulation)")
        elif st.session_state.get('backtest_save_result', '').startswith('error'):
            st.error(st.session_state['backtest_save_result'])
        elif st.session_state.get('backtest_save_result', '').startswith('warning'):
            st.warning(st.session_state['backtest_save_result'].replace('warning: ',''))
    ...
    with col2:
        ...
        try:
            ...
            FPDF = None
        generate_report_triggered = st.button("📊 Generate Report")
        if generate_report_triggered:
            st.session_state['pending_backtest_generate_report'] = True
        if st.session_state.get('pending_backtest_generate_report', False):
            if st.session_state.backtest_results:
                if FPDF is None:
                    st.session_state['backtest_report_result'] = 'error: PDF generation requires the fpdf package. Please install it with pip install fpdf.'
                else:
                    try:
                        results = st.session_state.backtest_results
                        pdf = FPDF()
                        ...
                        pdf.set_font("Arial", size=12)
                        pdf.multi_cell(0, 10, f"Total Runs: {len(results)}\nSymbol: {st.session_state.get('backtest_symbol', 'N/A')}")
                        pdf.ln(5)
                        pdf.set_font("Arial", 'B', 12)
                        ...
                        if isinstance(results, list) and isinstance(results[0], dict):
                            for i, res in enumerate(results):
                                pdf.cell(0, 8, f"Run {i+1}", ln=True)
                                for k, v in res.items():
                                    pdf.cell(0, 8, f"{k}: {v}", ln=True)
                                pdf.ln(2)
                        else:
                            pdf.cell(0, 8, str(results), ln=True)
                        pdf.ln(5)
                        pdf.set_font("Arial", 'B', 12)
                        pdf.cell(0, 10, "Risk Analysis & Recommendations", ln=True)
                        pdf.set_font("Arial", size=12)
                        pdf.multi_cell(0, 10, "(Add your own analysis here)")
                        pdf_bytes = pdf.output(dest='S')
                        st.session_state['backtest_report_pdf'] = pdf_bytes
                        st.session_state['backtest_report_result'] = 'success'
                    except Exception as e:
                        st.session_state['backtest_report_result'] = f'error: {e}'
            else:
                st.session_state['backtest_report_result'] = 'warning: No results to report!'
            st.session_state['pending_backtest_generate_report'] = False
        # Show last result
        if st.session_state.get('backtest_report_result') == 'success' and 'backtest_report_pdf' in st.session_state:
            st.success("PDF report generated!")
            st.download_button("Download Backtest Report PDF", st.session_state['backtest_report_pdf'], file_name="backtest_report.pdf", mime="application/pdf")
        elif st.session_state.get('backtest_report_result', '').startswith('error'):
            st.error(st.session_state['backtest_report_result'].replace('error: ',''))
        elif st.session_state.get('backtest_report_result', '').startswith('warning'):
            st.warning(st.session_state['backtest_report_result'].replace('warning: ',''))

# --- FOOTER: Visual indicator for Ultra-Fast Mode and system status ---
col1, col2, col3, col4 = st.columns(4)
with col1:
    ultra_refresh_triggered = st.button("🚀 ULTRA REFRESH", type="primary")
    if ultra_refresh_triggered:
...
with col2:
    ...
    st.markdown(
        f"<span style='font-size:1.1em;'>⚡ Ultra-Fast Mode: <b style='color:#00ff88'>{'ON' if ultra_fast_status else 'OFF'}</b></span>",
        unsafe_allow_html=True
    )
with col3:
    avg_response = st.session_state.perf_monitor.get_average_response_time()
    st.metric("⏱️ Avg Response", f"{avg_response:.1f}ms")
with col4:
    ...
    if binance_time:
        ...
        st.write(f"🕐 Local (UTC): {now.strftime('%H:%M:%S')}<br>🟡 Binance: {binance_time.strftime('%H:%M:%S')}<br>⏳ Diff: {diff:+.2f}s", unsafe_allow_html=True)
    else:
        st.write(f"🕐 Local (UTC): {now.strftime('%H:%M:%S')}<br>Binance: N/A", unsafe_allow_html=True)
